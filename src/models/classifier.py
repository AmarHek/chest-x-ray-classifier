import torch
import torch.nn as nn


# TODO: Add more classifier heads
class LinearClassifier(nn.Module):
    def __init__(self, in_features, num_classes):
        super(LinearClassifier, self).__init__()
        self.in_features = in_features
        self.num_classes = num_classes

        assert self.in_features > 0, "in_features must be greater than 0"
        assert self.num_classes > 0, "num_classes must be greater than 0"

        self.fc = nn.Linear(self.in_features, self.num_classes, bias=True)

    def forward(self, x):
        x = self.fc(x)
        return x


class CSRABase(nn.Module):
    # one basic block
    def __init__(self, input_dim, num_classes, T, lam):
        super(CSRABase, self).__init__()
        self.T = T      # temperature
        self.lam = lam  # Lambda
        self.head = nn.Conv2d(input_dim, num_classes, 1, bias=False)
        self.softmax = nn.Softmax(dim=2)

    def forward(self, x):
        # x (B d H W)
        # normalize classifier
        # score (B C HxW)
        score = self.head(x) / torch.norm(self.head.weight, dim=1, keepdim=True).transpose(0,1)
        score = score.flatten(2)
        base_logit = torch.mean(score, dim=2)

        if self.T == 99:  # max-pooling
            att_logit = torch.max(score, dim=2)[0]
        else:
            score_soft = self.softmax(score * self.T)
            att_logit = torch.sum(score * score_soft, dim=2)

        return base_logit + self.lam * att_logit


class CSRA(nn.Module):  # multi-head attention
    temp_settings = {  # softmax temperature settings
        1: [1],
        2: [1, 99],
        4: [1, 2, 4, 99],
        6: [1, 2, 3, 4, 5, 99],
        8: [1, 2, 3, 4, 5, 6, 7, 99]
    }

    def __init__(self, num_heads, lam, input_dim, num_classes):
        super(CSRA, self).__init__()
        self.temp_list = self.temp_settings[num_heads]
        self.multi_head = nn.ModuleList([
            CSRABase(input_dim, num_classes, self.temp_list[i], lam)
            for i in range(num_heads)
        ])

    def forward(self, x):
        logit = 0.
        for head in self.multi_head:
            logit += head(x)
        return logit


def classifier_head(head: str, in_features: int, num_classes, **kwargs):
    """
    Returns a classifier head given a string name.
    :param head:
    :param in_features:
    :param num_classes:
    :param kwargs:
    :return:
    """

    # get classifier head
    if head == "linear":
        return LinearClassifier(in_features, num_classes)
    elif head == "csra":
        return CSRA(in_features, num_classes, kwargs["num_heads"], kwargs["lam"])
    elif head == "none":
        return nn.Identity()
    else:
        raise NotImplementedError("Classifier head %s is not implemented!" % head)


def classifier_function(function_name: str):
    """
    Returns a classifier function given a string name.
    :param function_name:
    :return: The classifier function
    """

    functions = {
        "none": nn.Identity(),
        "sigmoid": nn.Sigmoid(),
        "softmax": nn.Softmax(),
        "logsoftmax": nn.LogSoftmax(),
        "logsigmoid": nn.LogSigmoid()
    }

    function_name = function_name.lower()

    assert function_name in functions.keys(), \
        "%s is an invalid classifier function!" % function_name

    return functions[function_name]
