...
for architecture in "${architectures[@]}"; do
  ...
  for classes in "${classes_sets[@]}"; do
    ...
    sbatch -p ls6 --gres=gpu:rtx2080ti:1 --wrap="python train1.py --architecture=$architecture --classes=$classes --model_path=$model_path$classes --csv_path=$csv_path --img_path=$img_path --image_size=$image_size --loss=$loss --optimizer=$optimizer --learning_rate=$learning_rate --batch_size=$batch_size --epochs=$epochs --lr_scheduler='reduce' --es_patience=5" -o $log_output
  done
done
