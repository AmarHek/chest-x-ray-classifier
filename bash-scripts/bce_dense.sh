...
for architecture in "${architectures[@]}"; do
  ...
  for classes in "${classes_sets[@]}"; do
    ...
    sbatch -p ls6 --gres=gpu:rtx3090:1 --wrap="python ../Scripts/train1.py $architecture $classes $model_path$classes $csv_path $img_path --image_size=$image_size --loss=$loss --optimizer=$optimizer --learning_rate=$learning_rate --batch_size=$batch_size --epochs=$epochs --lr_scheduler='plateau' --es_patience=5" -o $log_output
  done
done
